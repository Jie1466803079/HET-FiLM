...
# from IPython import embed  # compat-shim: removed, not used at runtime
...
def read_process_data_heter(dir, tokenizer, max_length, book_neighbour, shelves_neighbour, author_neighbour):
    ...
    with open(dir) as f:
        ...
        for line in tqdm(data):
            # MFP: upstream had `line.strip().split('\$\$')`. Two bugs in one
            # line for our funds adapter:
            #   1. The split target was a 4-char literal '\$\$'; our adapter
            #      writes plain '$$' as the query/key delimiter.
            #   2. strip() removes leading whitespace, so when the query's
            #      self_text is empty (common for funds with no prospectus
            #      text), the leading '\t' that delimits the empty first
            #      column gets eaten — turning a 14-col query side into 13.
            # Use rstrip('\n') instead to preserve leading delimiters.
            a = line.rstrip('\n').split('$$')
            if len(a) == 2:
                query_all, key_all = a
            else:
