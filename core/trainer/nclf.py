...
def train(
    ...
    for support, query in train_data:
        ...
        # Optional diversity regularizer from MGCell-like spaces
        try:
            mg_div_lambda = float(getattr(model, 'mg_div_lambda', 0.0))
        except Exception:
            mg_div_lambda = 0.0
        if mg_div_lambda > 0 and hasattr(model, 'spaces'):
            div_accum = 0.0
            n_terms = 0
            for sp in getattr(model, 'spaces', []):
                dl = getattr(sp, 'last_div_loss', None)
                if dl is not None and torch.is_tensor(dl):
                    div_accum = div_accum + dl
                    n_terms += 1
            if n_terms > 0:
                loss = loss + mg_div_lambda * (div_accum / n_terms)
        optimizer.zero_grad()
        loss.backward()
        if grad_clip > 0:
