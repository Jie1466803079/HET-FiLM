...
from core.models.ew_utils import use_ew_message, snapshot_edge_attr
...
class HTGNNLayer(nn.Module):
    def __init__(
        ...
        # EDGE_WEIGHT_MESSAGE=1 swaps in the message-scaling GAT conv
        # (EdgeWeightGATConv); default is the original GATConv, unchanged.
        self._use_ew_message = use_ew_message()
        if self._use_ew_message:
            from core.models.GAT import EdgeWeightGATConv as _ConvClass
        else:
            _ConvClass = GATConv
        ...
        self.post_rel_dropout = nn.Dropout(dropout)
    ...
    def forward(self, graphs, node_features):
        ...
        for ttype in self.timeframe:
            ...
            for ntype1, etype, ntype2 in self.etypes:
                ...
                xj = x[ntype2]
                if self._use_ew_message:
                    ew = snapshot_edge_attr(graph, etype)
                    out = self.intra_rel_agg[etype]((xi, xj), edge_index, edge_weight=ew)
                else:
