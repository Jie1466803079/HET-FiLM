"""
DHSpaceGRU: DHSpace with Node GRU and Edge GRU enhancements

Key improvements over DHSpace:
1. NodeGRU: Apply GRU to node features across time to capture temporal patterns before attention
2. EdgeGRU: For weighted edges, apply GRU on edge weight history to compute dynamic gates
3. Learned relation scalars: For unweighted edges, use learned scalars to modulate messages
4. Compatible with NAS: Architecture search for attention topology remains unchanged

Design:
- NodeGRU processes each node type's temporal sequence before DHSpace attention
- EdgeGRU produces gates for weighted edges (holds_stock, holds_other)
- Unweighted edges (managed_by) use learned relation-specific scalars
- All other DHSpace logic (attention, aggregation, NAS) remains the same
"""

from torch import nn
import torch.nn.functional as F
import torch
import numpy as np
import math
try:
    from torch_scatter import scatter
except ImportError:
    scatter = None
from torch_geometric.utils import softmax
from torch_geometric.nn.inits import glorot, ones
from .HLinear import HLinear, HLayerNorm


class DHSpaceGRU(nn.Module):
    def __init__(
        self,
        hid_dim,
        metadata,
        twin,
        K_To,
        K_N,
        K_R,
        n_heads=8,
        norm=True,
        causal_mask=False,
        last_mask=False,
        full_mask=False,
        node_hetero=False,
        node_entangle_type="None",
        rel_entangle_type="None",
        rel_time_type="relative",
        time_patch_num=1,
        hupdate=True,
        args=None,
        skip=False,
        **kwargs,
    ):
        super(DHSpaceGRU, self).__init__()

        # Data Attrs
        self.twin = twin

        # Data Type Info
        self.metadata = metadata
        self.node_types, self.edge_types = metadata
        self.num_types = len(self.node_types)
        self.num_relations = len(self.edge_types)
        self.ntype2id = dict(zip(self.node_types, range(self.num_types)))
        self.etype2id = dict(
            zip([et for _, et, _ in self.edge_types], range(self.num_relations))
        )
        self.id2ntype = self.node_types
        self.id2etype = self.edge_types

        # Space Constraint
        self.K_To = K_To
        self.K_N = K_N
        self.K_R = K_R
        self.causal_mask = causal_mask
        self.last_mask = last_mask
        self.full_mask = full_mask
        self.node_hetero = node_hetero
        self.norm = norm
        self.node_entangle_type = node_entangle_type
        self.rel_entangle_type = rel_entangle_type
        self.rel_time_type = rel_time_type
        self.collect_att = True
        self.time_patch_num = time_patch_num
        self.hupdate = hupdate
        self.args = args
        self.skip = skip
        
        # Model hyperparams
        self.hid_dim = hid_dim
        self.n_heads = n_heads
        self.d_k = hid_dim // n_heads
        self.sqrt_dk = math.sqrt(self.d_k)
        self.aggr = "add"
        self.node_dim = 0
        self.dropout = nn.Dropout(getattr(self.args, 'dropout', 0.0) if self.args is not None else 0.0)

        # NEW: Node GRUs for temporal encoding (one per node type)
        self.node_grus = nn.ModuleDict({
            ntype: nn.GRUCell(hid_dim, hid_dim)
            for ntype in self.node_types
        })

        # NEW: Edge GRUs and gates for weighted edges
        gate_hidden = getattr(self.args, 'gate_hidden', 16) if self.args is not None else 16
        self.edge_grus = nn.ModuleDict()
        self.edge_gate_heads = nn.ModuleDict()
        self.edge_weight_heads = nn.ModuleDict()  # NEW: for auxiliary weight prediction
        self.edge_zi_heads = nn.ModuleDict()  # NEW: for auxiliary zero-inflation
        self.unweighted_edge_scalars = nn.ParameterDict()
        
        for src_type, etype, dst_type in self.edge_types:
            rel_key = f"{src_type}_{etype}_{dst_type}"
            
            if "holds_" in etype:  # Weighted edges
                self.edge_grus[rel_key] = nn.GRU(
                    input_size=1,
                    hidden_size=gate_hidden,
                    batch_first=True
                )
                self.edge_gate_heads[rel_key] = nn.Linear(gate_hidden, 1)
                self.edge_weight_heads[rel_key] = nn.Linear(gate_hidden, 1)  # NEW
                self.edge_zi_heads[rel_key] = nn.Linear(gate_hidden, 1)  # NEW
            else:  # Unweighted edges - learned scalar
                self.unweighted_edge_scalars[rel_key] = nn.Parameter(torch.tensor(1.0))

        # init
        self.dummy_A_To = torch.ones(
            (twin, twin, self.num_relations)
        ).long()
        self.dummy_A_N = torch.zeros((twin, self.num_types)).long()

        if self.rel_time_type == "independent":
            rel_time_len = twin * twin
        elif self.rel_time_type == "relative":
            rel_time_len = 2 * twin
        elif self.rel_time_type == "source":
            rel_time_len = twin
        elif self.rel_time_type == "target":
            rel_time_len = twin
        else:
            raise NotImplementedError(f"Unknown rel_time_type {rel_time_type}")
        rel_time_len = twin * twin

        self.dummy_A_R = torch.zeros(
            (rel_time_len, self.num_relations)
        ).long()
        self.assign_arch(self.get_A_init())
        self.init_supernet()
        
        # NEW: Architecture search parameters (alphas)
        # These are needed for DHSearcher to work
        self.n_alpha = nn.Parameter(
            torch.randn((self.time_patch_num * self.num_types, K_N)) * 1e-3
        )
        self.r_alpha = nn.Parameter(
            torch.randn((self.time_patch_num * self.num_relations, K_R)) * 1e-3
        )
        self.to_alpha = nn.Parameter(
            torch.randn((self.twin, self.twin, self.num_relations)) * 1e-3
        )

        # External edge features (for EdgeGRU)
        self.edge_features = {}

    def set_edge_features(self, edge_features):
        """Set edge features dict before forward pass"""
        self.edge_features = edge_features or {}

    def init_supernet(self, fixed=False):
        hid_dim = self.hid_dim
        twin = self.twin
        K_To = self.K_To
        K_N = self.K_N
        K_R = self.K_R

        n_heads = self.n_heads
        d_k = self.d_k

        # Message
        def makelinear(n):
            linears = nn.ModuleList()
            for ntype in range(n):
                linears.append(nn.Linear(hid_dim, hid_dim))
            return linears

        self.k_linears = makelinear(K_N)
        self.q_linears = makelinear(K_N)
        self.v_linears = makelinear(K_N)

        # Aggregate
        self.relation_pri = nn.Parameter(torch.ones(K_R, n_heads))
        self.relation_att = nn.Parameter(torch.Tensor(K_R, n_heads, d_k, d_k))
        self.relation_msg = nn.Parameter(torch.Tensor(K_R, n_heads, d_k, d_k))

        ones(self.relation_pri)
        glorot(self.relation_att)
        glorot(self.relation_msg)

        if self.hupdate:
            self.update_lin = HLinear(hid_dim, self.metadata, act="None")
        if self.norm:
            self.update_norm = HLayerNorm(hid_dim, self.metadata)

    def reset_parameters(self):
        for ls in [self.k_linears, self.q_linears, self.v_linears]:
            for l in ls:
                l.reset_parameters()

        ones(self.relation_pri)
        glorot(self.relation_att)
        glorot(self.relation_msg)

        if self.hupdate:
            self.update_lin.reset_parameters()
        if self.norm:
            self.update_norm.reset_parameters()
        
        # Reset GRUs
        for gru in self.node_grus.values():
            gru.reset_parameters()
        for gru in self.edge_grus.values():
            gru.reset_parameters()

    def get_A_init(self):
        return (self.dummy_A_To.clone(), self.dummy_A_N.clone(), self.dummy_A_R.clone())

    def get_arch(self):
        return [a.clone() for a in self.A]

    def list2mat_A(self, ls):
        lTo, lN, lR = ls
        ATo, AN, AR = self.get_A_init()
        for i, j, k in lTo:
            ATo[i, j, k] = 1
        for i, j, k in lN:
            AN[i, j] = k
        for i, j, k in lR:
            AR[i, j] = k
        return ATo, AN, AR

    def mat2list_A(self, As):
        twin = self.twin
        num_types = self.num_types
        num_relations = self.num_relations
        ATo, AN, AR = As
        lTo, lN, lR = [], [], []
        for i in range(twin):
            for j in range(twin):
                for k in range(num_relations):
                    if ATo[i, j, k]:
                        lTo.append((i, j, k))
        for i in range(twin):
            for j in range(num_types):
                k = AN[i, j]
                lN.append((i, j, k.item()))
        for i in range(2 * twin):
            for j in range(num_relations):
                k = AR[i, j]
                lR.append((i, j, k.item()))
        return lTo, lN, lR

    def assign_arch(self, A):
        if isinstance(A[0], list):
            A = self.list2mat_A(A)
        self.A = A
        return self

    def assign_basic_arch(self, atype):
        return self.assign_arch(self.basic_space(atype))

    def basic_space(self, atype):
        ATo, AN, AR = self.get_A_init()
        twin = self.twin
        num_relations = self.num_relations
        num_types = self.num_types
        if "full" in atype:
            ATo = ATo.new_ones(ATo.shape)
        if "last" in atype:
            ATo = ATo.new_zeros(ATo.shape)
            ATo[-1, :, :] = 1
        if "causal" in atype:
            ATo = ATo.new_ones(ATo.shape)
            for i in range(twin):
                for j in range(i + 1, twin):
                    for k in range(num_relations):
                        ATo[i, j, k] = 0
        if "node_hetero" in atype:
            AN = AN.new_zeros(AN.shape)
            for i in range(twin):
                for j in range(num_types):
                    AN[i, j] = j
        if "rel_hetero" in atype:
            AR = AR.new_zeros(AR.shape)
            for i in range(2 * twin):
                for j in range(num_relations):
                    AR[i, j] = j

        return (ATo, AN, AR)

    def count_space(self):
        ATo, AN, AR = self.A
        kto = ATo.sum().item()
        kn = len(AN.unique())
        kr = len(AR.unique())
        return kto, kn, kr

    def get_node_mapping_weights(self):
        """return shape KN x ((hdim*hdim+hdim)*3)"""
        def get_lin_weight(lin):
            weight = (
                torch.cat([lin.weight.flatten(), lin.bias], dim=0).to("cpu").detach()
            )
            return weight

        def get_lin_weights(lins):
            weights = torch.stack([get_lin_weight(lin) for lin in lins], dim=0)
            weights = weights.numpy()
            return weights

        weights = np.concatenate(
            [
                get_lin_weights(lins)
                for lins in [self.q_linears, self.k_linears, self.v_linears]
            ],
            axis=1,
        )
        return weights

    def assign_node_mapping_weights(self, idx, weights):
        """weights shape ((hdim*hdim+hdim)*3)"""
        def assign_weight(lin, weights):
            hid_dim, hid_dim = lin.weight.shape
            device = lin.weight.device
            weight = nn.Parameter(
                torch.FloatTensor(
                    weights[: hid_dim * hid_dim].reshape((hid_dim, hid_dim))
                ).to(device)
            )
            bias = nn.Parameter(
                torch.FloatTensor(weights[hid_dim * hid_dim :]).to(device)
            )
            lin.weight = weight
            lin.bias = bias

        dim = weights.shape[0] // 3
        for i, lin in enumerate([self.q_linears, self.k_linears, self.v_linears]):
            assign_weight(lin[idx], weights[i * dim : (i + 1) * dim])

    def get_modules_weights(self, target_type, source_type, ttar, tsrc, relation_type):
        _, AN, AR = self.A
        source_type = self.ntype2id[source_type]
        target_type = self.ntype2id[target_type]
        relation_type = self.etype2id[relation_type]
        FNsrc = AN[tsrc, source_type]
        FNtar = AN[ttar, target_type]
        if self.rel_time_type == "relative":
            dt = tsrc - ttar + self.twin - 1
        elif self.rel_time_type == "independent":
            dt = ttar * self.twin + tsrc
        elif self.rel_time_type == "source":
            dt = tsrc
        elif self.rel_time_type == "target":
            dt = ttar
        else:
            raise NotImplementedError(f"Unknown rel_time_type {self.rel_time_type}")

        q_linear = self.q_linears[FNtar]
        k_linear = self.k_linears[FNsrc]
        v_linear = self.v_linears[FNsrc]

        def combine(funcs, agg="add"):
            def aggfunc(*args, **kwargs):
                x = funcs[0](*args, **kwargs)
                for i in range(1, len(funcs)):
                    if agg == "add":
                        x = x + funcs[i](*args, **kwargs)
                return x

            return aggfunc

        FR = AR[dt, relation_type]

        relation_att = self.relation_att[FR]
        relation_pri = self.relation_pri[FR]
        relation_msg = self.relation_msg[FR]
        return q_linear, k_linear, v_linear, relation_att, relation_pri, relation_msg

    def compute_edge_gates(self, rel_key, edge_index, t_src):
        """
        Compute gates for edges based on historical weights (for weighted edges)
        or return learned scalar (for unweighted edges)
        
        Args:
            rel_key: relation identifier string
            edge_index: edge indices [2, E]
            t_src: source time index (for causal alignment)
            
        Returns:
            gates: [E] gate values to multiply with messages
            w_pred: [E] weight predictions for auxiliary loss (None if unweighted)
            zi_pred: [E] zero-inflation logits for auxiliary loss (None if unweighted)
        """
        E = edge_index.size(1)
        device = edge_index.device
        
        if E == 0:
            return torch.ones(0, device=device), None, None
        
        # Check if this is a weighted edge type
        if rel_key in self.edge_grus:
            # EdgeGRU for weighted edges
            feats = self.edge_features.get(rel_key, None)
            if feats is None or 'window' not in feats:
                # No edge features available - default to pass-through
                return torch.ones(E, device=device), None, None
            
            # Get historical weight window [E, L] or [E, L, 1]
            # Use only history up to t_src-1 for causality
            window = feats['window'].to(device)
            if window.dim() == 2:
                window = window.unsqueeze(-1)  # [E, L, 1]
            
            # Run EdgeGRU
            _, hidden = self.edge_grus[rel_key](window)  # [1, E, H]
            hidden = hidden.squeeze(0)  # [E, H]
            
            # Predict gate values
            gates = torch.sigmoid(self.edge_gate_heads[rel_key](hidden).squeeze(-1))  # [E]
            
            # NEW: Auxiliary predictions
            w_pred = self.edge_weight_heads[rel_key](hidden).squeeze(-1)  # [E]
            zi_pred = self.edge_zi_heads[rel_key](hidden).squeeze(-1)  # [E]
            
            # Clamp for stability
            gate_clip = getattr(self.args, 'gate_clip', (0.02, 0.98)) if self.args is not None else (0.02, 0.98)
            gates = gates.clamp(gate_clip[0], gate_clip[1])
            
        else:
            # Learned scalar for unweighted edges
            scalar = self.unweighted_edge_scalars[rel_key]
            gates = torch.full((E,), scalar.item(), device=device)
            w_pred = None
            zi_pred = None
        
        return gates, w_pred, zi_pred

    def DHAttn(self, x_tar_rel, x_src_rel, t_tar, t_src, rel, edge_index):
        """
        DHSpace attention with edge gating
        
        @return att [E,h] ; msg [E,h,F/h] ; gates [E] ; w_pred [E] ; zi_pred [E]
        """
        target_node_vec = x_tar_rel
        source_node_vec = x_src_rel

        # get weights
        source_type, _, target_type = self.id2etype[self.etype2id[rel]]
        (
            q_linear,
            k_linear,
            v_linear,
            relation_att,
            relation_pri,
            relation_msg,
        ) = self.get_modules_weights(target_type, source_type, t_tar, t_src, rel)

        # calculation
        q_mat = q_linear(target_node_vec).view(-1, self.n_heads, self.d_k)
        k_mat = k_linear(source_node_vec).view(-1, self.n_heads, self.d_k)
        v_mat = v_linear(source_node_vec).view(-1, self.n_heads, self.d_k)

        k_mat = torch.bmm(k_mat.transpose(1, 0), relation_att).transpose(1, 0)
        res_att = (q_mat * k_mat).sum(dim=-1) * relation_pri / self.sqrt_dk  # [E,h]
        res_msg = torch.bmm(v_mat.transpose(1, 0), relation_msg).transpose(
            1, 0
        )  # [E,h,F/h]
        
        # NEW: Compute edge gates and auxiliary predictions
        src_type, etype, dst_type = self.id2etype[self.etype2id[rel]]
        rel_key = f"{src_type}_{etype}_{dst_type}"
        gates, w_pred, zi_pred = self.compute_edge_gates(rel_key, edge_index, t_src)
        
        return res_att, res_msg, gates, w_pred, zi_pred

    def DHAttnOne2Multi(self, x_tar, topos):
        # message
        res_atts = {}
        res_msgs = {}
        res_gates = {}
        res_w_preds = {}  # NEW: collect weight predictions
        res_zi_preds = {}  # NEW: collect zero-inflation predictions
        ei_tars = {}
        aux_outputs = {}  # NEW: auxiliary outputs for loss computation
        
        for (x_tar_rel, x_src_rel, t_tar, t_src, rel, ei_rel_tar, ei_rel) in topos:
            _, _, target_type = self.id2etype[self.etype2id[rel]]
            for collect in [res_atts, res_msgs, res_gates, res_w_preds, res_zi_preds, ei_tars]:
                if target_type not in collect:
                    collect[target_type] = []
            att, msg, gates, w_pred, zi_pred = self.DHAttn(x_tar_rel, x_src_rel, t_tar, t_src, rel, ei_rel)

            res_atts[target_type].append(att)
            res_msgs[target_type].append(msg)
            res_gates[target_type].append(gates)
            res_w_preds[target_type].append(w_pred)
            res_zi_preds[target_type].append(zi_pred)
            ei_tars[target_type].append(ei_rel_tar)
            
            # Store auxiliary predictions per relation for loss computation
            src_type, etype, dst_type = self.id2etype[self.etype2id[rel]]
            rel_key = f"{src_type}_{etype}_{dst_type}"
            if w_pred is not None:
                if rel_key not in aux_outputs:
                    aux_outputs[rel_key] = {'w_pred': [], 'zi_pred': []}
                aux_outputs[rel_key]['w_pred'].append(w_pred)
                aux_outputs[rel_key]['zi_pred'].append(zi_pred)

        x_dict = {}
        for ntype in self.node_types:
            if ntype in res_atts:
                # aggregate
                res_att = torch.cat(res_atts[ntype], dim=0)
                res_msg = torch.cat(res_msgs[ntype], dim=0)
                res_gate = torch.cat(res_gates[ntype], dim=0)
                ei_tar = torch.cat(ei_tars[ntype])

                res_att = softmax(res_att, ei_tar)  # [ET,h] - normalized per destination
                
                # NEW: Normalize gates per destination for stable message scale
                # This makes gates interpretable as relative importance weights
                res_gate_norm = res_gate / (scatter(
                    res_gate,
                    ei_tar,
                    dim=0,
                    dim_size=x_tar[ntype].shape[0],
                    reduce='sum'
                )[ei_tar] + 1e-8)  # [E] normalized so sum per dest = 1
                
                # Apply normalized gates to messages (post-attention gating)
                res = res_msg * res_att.view(-1, self.n_heads, 1)  # [E,h,F/h]
                res = res * res_gate_norm.view(-1, 1, 1)  # [E,h,F/h] * [E,1,1]

                del res_atts[ntype], res_msgs[ntype], res_gates[ntype]

                res = res.view(-1, self.hid_dim)  # [E,F]
                res = scatter(
                    res,
                    ei_tar,
                    dim=self.node_dim,
                    dim_size=x_tar[ntype].shape[0],
                    reduce=self.aggr,
                )  # [N,F]
                del ei_tars[ntype]

                # update
                if self.hupdate:
                    res = self.update_lin[ntype](F.gelu(res))
                res = x_tar[ntype] + self.dropout(res)
            else:
                # aggregate nothing
                res = x_tar[ntype]
            x_dict[ntype] = res
        
        # Concatenate auxiliary predictions for loss computation
        for rel_key in aux_outputs:
            if aux_outputs[rel_key]['w_pred']:
                aux_outputs[rel_key]['w_pred'] = torch.cat(aux_outputs[rel_key]['w_pred'])
                aux_outputs[rel_key]['zi_pred'] = torch.cat(aux_outputs[rel_key]['zi_pred'])
        
        return x_dict, aux_outputs

    def forward(self, xs, graphs):
        """
        Forward pass with NodeGRU temporal encoding
        
        Args:
            xs: list of x_dict, one per time step [time, ntype] -> [N, F]
            graphs: list of graph objects, one per time step
            
        Returns:
            x_res: list of updated node representations [time, ntype] -> [N, F]
            aux_outputs_all: dict of auxiliary predictions for loss computation
        """
        twin = self.twin
        device = xs[0][self.id2ntype[0]].device

        # NEW: Apply NodeGRU across time dimension first
        # Initialize hidden states
        h_gru = {ntype: None for ntype in self.node_types}
        
        # Process each timestep with NodeGRU
        x_gru_processed = []
        for t in range(twin):
            x_t = xs[t]
            x_t_updated = {}
            for ntype in self.node_types:
                x_node = x_t[ntype]
                if h_gru[ntype] is None:
                    # First timestep: initialize with zeros
                    h_gru[ntype] = torch.zeros_like(x_node)
                # GRU update
                h_gru[ntype] = self.node_grus[ntype](x_node, h_gru[ntype])
                x_t_updated[ntype] = h_gru[ntype]
            x_gru_processed.append(x_t_updated)
        
        # Now apply DHSpace attention on GRU-processed features
        x_win = x_gru_processed
        x_res = []
        aux_outputs_all = {}  # Collect all auxiliary outputs

        # calculation
        ATo, _, _ = self.A
        ATo = ATo.to(device)
        for t_tar in range(twin):
            # get topo need to process
            ATo_tar = ATo[t_tar]
            if ATo_tar.sum() == 0:  # no need : use original
                x_dict = x_win[t_tar]
            else:
                topos = []
                x_tar = x_win[t_tar]
                # select all attention in need of computation w.r.t topology
                for t_src in range(twin):
                    if ATo_tar[t_src].sum() == 0:
                        continue
                    graph_src = graphs[t_src]
                    x_src = x_win[t_src]
                    for rel in ATo_tar[t_src].nonzero():
                        nsrc, rel, ntar = self.id2etype[rel]
                        ei_rel = graph_src[rel].edge_index
                        x_tar_rel = x_tar[ntar].index_select(
                            self.node_dim, ei_rel[1, :]
                        )
                        x_src_rel = x_src[nsrc].index_select(
                            self.node_dim, ei_rel[0, :]
                        )
                        ei_rel_tar = ei_rel[1, :].T
                        topo = (x_tar_rel, x_src_rel, t_tar, t_src, rel, ei_rel_tar, ei_rel)
                        topos.append(topo)
                # calculate
                x_dict, aux_outputs = self.DHAttnOne2Multi(x_tar, topos)
                
                # Accumulate auxiliary outputs across time steps
                for rel_key in aux_outputs:
                    if rel_key not in aux_outputs_all:
                        aux_outputs_all[rel_key] = {'w_pred': [], 'zi_pred': []}
                    aux_outputs_all[rel_key]['w_pred'].append(aux_outputs[rel_key]['w_pred'])
                    aux_outputs_all[rel_key]['zi_pred'].append(aux_outputs[rel_key]['zi_pred'])
                
                if self.norm:
                    x_dict = self.update_norm(x_dict)
            x_res.append(x_dict)
        
        # Concatenate auxiliary predictions across all time steps
        for rel_key in aux_outputs_all:
            if aux_outputs_all[rel_key]['w_pred']:
                aux_outputs_all[rel_key]['w_pred'] = torch.cat(aux_outputs_all[rel_key]['w_pred'])
                aux_outputs_all[rel_key]['zi_pred'] = torch.cat(aux_outputs_all[rel_key]['zi_pred'])
        
        return x_res, aux_outputs_all

    @torch.no_grad()
    def fix_n_alpha(self):
        """Fix node alpha parameters after architecture search"""
        AN = self.A[1]
        for i in range(AN.shape[0]):
            for j in range(AN.shape[1]):
                alpha = self.n_alpha[AN[i, j]]
                AN[i, j] = torch.argmax(alpha).item()

    @torch.no_grad()
    def fix_r_alpha(self):
        """Fix relation alpha parameters after architecture search"""
        AR = self.A[2]
        for i in range(AR.shape[0]):
            for j in range(AR.shape[1]):
                alpha = self.r_alpha[AR[i, j]]
                AR[i, j] = torch.argmax(alpha).item()

    @torch.no_grad()
    def fix_to_alpha(self):
        """Fix temporal alpha parameters after architecture search"""
        K_To = self.K_To
        to_alpha = self.to_alpha  # [T,T,num_relations]
        twin = self.twin
        num_relations = self.num_relations
        ATo = self.A[0].clone()
        for t in range(twin):
            for i in range(twin):
                for r in range(num_relations):
                    self.A[0][t, i, r] = 0

        for t in range(twin):
            valid_alpha = []
            for i in range(twin):
                for r in range(num_relations):
                    if ATo[t, i, r]:
                        valid_alpha.append([to_alpha[t, i, r].item(), i, r])
            valid_alpha = sorted(valid_alpha, key=lambda x: x[0], reverse=True)
            soft_alpha = torch.FloatTensor([x[0] for x in valid_alpha])
            soft_alpha = F.softmax(soft_alpha, dim=0)
            
            # Keep top K_To connections
            for i in range(min(K_To, len(valid_alpha))):
                _, ti, ri = valid_alpha[i]
                self.A[0][t, ti, ri] = 1

    def setATo(self, atype="full"):
        """Set temporal attention topology"""
        twin = self.twin
        num_relations = self.num_relations
        if atype == "full":
            ATo = self.A[0]
            for i in range(twin):
                for j in range(twin):
                    for k in range(num_relations):
                        ATo[i, j, k] = 1
        elif atype == "causal":
            ATo = self.A[0]
            for i in range(twin):
                for j in range(twin):
                    for k in range(num_relations):
                        if j <= i:
                            ATo[i, j, k] = 1
                        else:
                            ATo[i, j, k] = 0
        else:
            raise NotImplementedError(f"Unknown ATo atype {atype}")

    def setAN(self, atype="same"):
        """Set node architecture to match DHSpace semantics."""
        twin = self.twin
        num_types = self.num_types
        patch_num = self.time_patch_num
        part = int(np.ceil(twin / patch_num))
        AN = self.A[1]
        if atype == "same":
            for i in range(twin):
                for j in range(num_types):
                    AN[i, j] = 0
        elif atype == "hetero":
            for i in range(twin):
                for j in range(num_types):
                    AN[i, j] = j
        elif atype == "t-hetero":
            for i in range(patch_num):
                for j in range(num_types):
                    AN[i * part : i * part + part, j] = i * num_types + j
        else:
            raise NotImplementedError(f"Unknown AN atype {atype}")

    def setAR(self, atype="same"):
        """Set relation architecture to match DHSpace semantics."""
        twin = self.twin
        num_relations = self.num_relations
        patch_num = self.time_patch_num
        part = int(np.ceil(twin / patch_num))
        AR = self.A[2]
        if atype == "same":
            for i in range(twin):
                for j in range(num_relations):
                    AR[i, j] = 0
        elif atype == "hetero":
            for i in range(twin):
                for j in range(num_relations):
                    AR[i, j] = j
        elif atype == "t-hetero":
            for i in range(patch_num):
                for j in range(num_relations):
                    AR[i * part : i * part + part, j] = i * num_relations + j
        else:
            raise NotImplementedError(f"Unknown AR atype {atype}")

    def set_stage(self, stage):
        """Set architecture search stage"""
        print(f'{"#"*10} stage {stage} {"#"*100}')
        if stage == 0:
            self.setATo("full")
            self.fix_To = True
            self.setAN("same")
            self.fix_N = True
            self.setAR("same")
            self.fix_R = True
        elif stage == 1:
            self.setATo("full")
            self.fix_To = True
            self.setAN("same")
            self.fix_N = False
            self.setAR("same")
            self.fix_R = False
        elif stage == 2:
            self.setATo("full")
            self.fix_To = False
            self.setAN("same")
            self.fix_N = False
            self.setAR("same")
            self.fix_R = False

    def auxiliary_losses(self, aux_outputs, edge_features):
        """
        Compute auxiliary losses for weight prediction and zero-inflation
        
        Args:
            aux_outputs: dict from forward pass {rel_key: {'w_pred': [E], 'zi_pred': [E]}}
            edge_features: dict with target labels {rel_key: {'w_next': [E], 'weight': [E]}}
            
        Returns:
            scalar loss value
        """
        device = next(self.parameters()).device
        losses = []
        
        for rel_key, preds in aux_outputs.items():
            if "holds_" not in rel_key:
                continue
            
            feats = edge_features.get(rel_key, None)
            if feats is None:
                continue
            
            w_pred = preds.get('w_pred', None)
            zi_pred = preds.get('zi_pred', None)
            
            if w_pred is None or zi_pred is None:
                continue
            
            # Get target weights
            use_next = getattr(self.args, 'use_next_weight_aux', True) if self.args is not None else True
            w_next = feats.get('w_next', None)
            w_cur = feats.get('weight', None)
            
            target_full = None
            if use_next and (w_next is not None):
                target_full = w_next.to(device)
            elif w_cur is not None:
                target_full = w_cur.to(device)
            
            if target_full is None:
                continue
            
            # Align sizes (predictions may be from multiple time steps, concatenated)
            m = min(w_pred.numel(), target_full.numel())
            if m == 0:
                continue
            
            w_pred = w_pred[:m]
            zi_pred = zi_pred[:m]
            target = target_full[:m]
            
            # Weight regression (MSE)
            losses.append(F.mse_loss(w_pred, target))
            
            # Zero-inflation (BCE)
            exist = (target > 0).float()
            losses.append(F.binary_cross_entropy_with_logits(zi_pred, exist))
        
        if not losses:
            return torch.tensor(0.0, device=device)
        return torch.stack(losses).mean()

    def __repr__(self):
        return f"""{super().__repr__()} with 
        metadict : {self.node_types,self.edge_types}
        Cn,Ce,T : {self.num_types,self.num_relations,self.twin}
        KTo,KN,KR : {self.K_To,self.K_N,self.K_R}
        hiddim,n_heads,d_k : {self.hid_dim,self.n_heads,self.d_k}
        A : {self.mat2list_A(self.A)}
        NodeGRU: enabled for all node types
        EdgeGRU: enabled for weighted edges (holds_*)
        Unweighted: learned scalars
        """


def HAct(x_dict, act=F.relu):
    if isinstance(x_dict, list):
        for x in x_dict:
            HAct(x, act)
    else:
        for ntype in x_dict:
            x_dict[ntype] = act(x_dict[ntype])
    return x_dict


class DHNetGRU(nn.Module):
    """
    DHNet wrapper with GRU enhancements
    Compatible with architecture search
    """
    def __init__(
        self,
        hid_dim,
        twin,
        metadata,
        spaces,
        predict_type,
        featemb=None,
        nclf_linear=None,
        hlinear_act="tanh",
        agg="last",
    ):
        super(DHNetGRU, self).__init__()

        # Data Attr
        self.hid_dim = hid_dim
        self.metadata = metadata
        self.twin = twin
        self.predict_type = predict_type

        # Searchable Archs
        self.spaces = nn.ModuleList(spaces)

        # Fixed Backbone
        self.hlinear = HLinear(hid_dim, metadata, act=hlinear_act)
        self.featemb = featemb if featemb else lambda x: x
        self.act = lambda x: HAct(x, F.relu)
        self.nclf_linear = nclf_linear
        self.agg = agg

    def reset_parameters(self, tp=None):
        if not tp:
            tp = ["space", "hlinear", "featemb"]
        for t in tp:
            if t not in ["space", "hlinear", "featemb"]:
                raise NotImplementedError(f"Unknown type {t}")
        if "space" in tp:
            for space in self.spaces:
                space.reset_parameters()
        if "hlinear" in tp:
            self.hlinear.reset_parameters()
        if "featemb" in tp:
            self.featemb.reset_parameters()

    def freeze(self, module_names):
        for name in module_names:
            assert name in "feat_emb hlinear hupdate hnorm rel".split(), print(
                f"Unimplemented {module_names}"
            )

        if "feat_emb" in module_names:
            for param in self.featemb.parameters():
                param.requires_grad = False
        if "hlinear" in module_names:
            for param in self.hlinear.parameters():
                param.requires_grad = False

        if "hupdate" in module_names:
            for space in self.spaces:
                if space.hupdate:
                    for param in space.update_lin.parameters():
                        param.requires_grad = False

        if "hnorm" in module_names:
            for space in self.spaces:
                if space.norm:
                    for param in space.update_norm.parameters():
                        param.requires_grad = False

        if "rel" in module_names:
            for space in self.spaces:
                for rel in [space.relation_att, space.relation_pri, space.relation_msg]:
                    rel.requires_grad = False

    def assign_arch(self, As):
        for i, space in enumerate(self.spaces):
            space.assign_arch(As[i])
        return self

    def get_configs(self):
        return [space.get_arch() for space in self.spaces]

    def count_space(self):
        ks = np.array([0, 0, 0])
        for space in self.spaces:
            k = np.array(list(space.count_space()))
            ks += k
        return list(map(int, ks))

    def set_edge_features(self, edge_features):
        """Propagate edge features to all spaces"""
        for space in self.spaces:
            space.set_edge_features(edge_features)

    def encode(self, graphs, *args, **kwargs):
        """
        Encode method for DHNetGRU
        
        Returns:
            out: node embeddings for predict_type
            aux_outputs_all: dict of auxiliary predictions (if spaces return them)
        """
        xs = []
        for graph in graphs:
            x_dict = self.featemb(graph.x_dict)
            x_dict = self.hlinear(x_dict)
            xs.append(x_dict)
        out = xs
        aux_outputs_all = {}
        
        for i, conv in enumerate(self.spaces):
            conv_out = conv(out, graphs)
            # Check if conv returns tuple (out, aux_outputs)
            if isinstance(conv_out, tuple) and len(conv_out) == 2:
                out, aux_outputs = conv_out
                # Accumulate auxiliary outputs
                for rel_key in aux_outputs:
                    if rel_key not in aux_outputs_all:
                        aux_outputs_all[rel_key] = {'w_pred': [], 'zi_pred': []}
                    aux_outputs_all[rel_key]['w_pred'].append(aux_outputs[rel_key]['w_pred'])
                    aux_outputs_all[rel_key]['zi_pred'].append(aux_outputs[rel_key]['zi_pred'])
            else:
                out = conv_out
            
            if i != len(self.spaces) - 1:
                out = self.act(out)

        # Concatenate auxiliary outputs if any
        for rel_key in aux_outputs_all:
            if aux_outputs_all[rel_key]['w_pred']:
                aux_outputs_all[rel_key]['w_pred'] = torch.cat(aux_outputs_all[rel_key]['w_pred'])
                aux_outputs_all[rel_key]['zi_pred'] = torch.cat(aux_outputs_all[rel_key]['zi_pred'])

        predict_type = self.predict_type
        if self.agg == "last":
            out = out[-1]
            if isinstance(predict_type, list):
                out = [out[predict_type[0]], out[predict_type[1]]]
            else:
                out = out[predict_type]
        elif self.agg == "sum":
            out = out[-1]
            if isinstance(predict_type, list):
                out = [
                    sum(x[predict_type[0]] for x in out),
                    sum(x[predict_type[1]] for x in out),
                ]
            else:
                out = sum(x[predict_type] for x in out)
        
        # Return embeddings and auxiliary outputs
        if aux_outputs_all:
            return out, aux_outputs_all
        return out

    def decode(self, z, edge_label_index):
        if isinstance(z, list) or isinstance(z, tuple):
            return (z[0][edge_label_index[0]] * z[1][edge_label_index[1]]).sum(dim=-1)
        return (z[edge_label_index[0]] * z[edge_label_index[1]]).sum(dim=-1)

    def decode_nclf(self, z):
        out = self.nclf_linear(z)
        return out

    def auxiliary_losses(self, aux_outputs, edge_features):
        """
        Compute auxiliary losses by delegating to spaces
        
        Args:
            aux_outputs: dict from encode/forward
            edge_features: dict with target labels
            
        Returns:
            scalar loss value
        """
        losses = []
        for space in self.spaces:
            if hasattr(space, 'auxiliary_losses'):
                loss = space.auxiliary_losses(aux_outputs, edge_features)
                if loss is not None and loss.item() > 0:
                    losses.append(loss)
        
        if not losses:
            return torch.tensor(0.0, device=next(self.parameters()).device)
        return torch.stack(losses).mean()

