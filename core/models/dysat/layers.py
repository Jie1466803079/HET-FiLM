...
class StructuralAttentionLayer2(nn.Module):
    ...
    def forward(self, x, graph):
        ...
        # Raw edge-weight message scaling (EDGE_WEIGHT_MESSAGE=1): DySAT.encode
        # attaches graph.edge_weight aligned with edge_index; absent otherwise.
        ew = getattr(graph, "edge_weight", None)
        if ew is not None:
            x_j = x_j * ew.view(-1, 1, 1)
