...
class DyHATRLayer(nn.Module):
    def __init__(
        ...
        self.post_time_dropout = nn.Dropout(dropout)
    ...
    def forward(self, graphs, node_features):
        ...
        for ttype in intra_features.keys():
            for ntype in self.ntypes:
                ...
                inter_features[ntype][ttype] = self.post_rel_dropout(out_feat)
        # time
        ...
        def timeaggr(feats):
            ...
            feats = self.post_time_dropout(feats)
            return feats
...
class DyHATR(nn.Module):
    def __init__(
        ...
        # input projection per node type with configurable activation
        self.hlinear = HLinear(n_hid, metadata, act=hlinear_act)
