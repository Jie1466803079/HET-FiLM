...
class TemporalGRUEncoder(nn.Module):
    """
    GRU-based temporal encoder for neighbor sequences.
    Processes node representations across time to capture temporal dynamics.
    """
    def __init__(self, hid_dim, num_node_types, bidirectional=False):
        super(TemporalGRUEncoder, self).__init__()
        self.hid_dim = hid_dim
        self.num_node_types = num_node_types
        self.bidirectional = bidirectional

        # Separate GRU for each node type to preserve heterogeneity
        self.grus = nn.ModuleDict()
        for ntype_id in range(num_node_types):
            self.grus[str(ntype_id)] = nn.GRU(
                input_size=hid_dim,
                hidden_size=hid_dim,
                num_layers=1,
        ...
        # Linear projection if bidirectional to match dimensions
        if bidirectional:
            self.proj = nn.ModuleDict()
            for ntype_id in range(num_node_types):
                self.proj[str(ntype_id)] = nn.Linear(hid_dim * 2, hid_dim)

    def forward(self, x_seq, ntype_id):
        """
        Args:
            x_seq: [T, N, F] tensor of node features across time
            ntype_id: int, node type index
        Returns:
            encoded_seq: [T, N, F] GRU-encoded temporal features
        """
        gru = self.grus[str(ntype_id)]
        # x_seq: [T, N, F]
        output, _ = gru(x_seq)  # output: [T, N, F] or [T, N, 2F] if bidirectional

        if self.bidirectional:
            # Project back to original dimension
            proj = self.proj[str(ntype_id)]
            T, N, _ = output.shape
            output = proj(output.reshape(-1, self.hid_dim * 2)).reshape(T, N, self.hid_dim)

        return output


class DHSpace_GRU(nn.Module):
    """
    Hybrid GRU + CORE model.
    Uses GRU to encode temporal neighbor sequences, then applies CORE attention on top.
    """
    def __init__(
        self,
        hid_dim,
        ...
        **kwargs,
        ...
        super(DHSpace_GRU, self).__init__()
    ...
    def init_supernet(self, fixed=False):
        ...
        # Architecture parameters for NAS (optional, only used during search)
        if self.args is not None and hasattr(self.args, 'enable_search') and self.args.enable_search:
    ...
    def reset_parameters(self):
        ...
        if self.use_gru:
            for gru in self.temporal_gru.grus.values():
    ...
    def encode_temporal_with_gru(self, xs):
        """
        Apply GRU encoding to temporal sequences for each node type.
        Args:
            xs: List of dicts [time,ntype] -> [N,F]
        Returns:
            xs_encoded: List of dicts with GRU-encoded features
        """
        if not self.use_gru:
            return xs

        # Organize by node type first: {ntype: [T, N, F]}
        ntype_sequences = {}
        for ntype in self.node_types:
            ntype_id = self.ntype2id[ntype]
            # Stack across time: [T, N, F]
            seq = torch.stack([x[ntype] for x in xs], dim=0)
            # Apply GRU encoding
            encoded_seq = self.temporal_gru(seq, ntype_id)
            ntype_sequences[ntype] = encoded_seq
        ...
        for t in range(self.twin):
            x_dict = {}
            for ntype in self.node_types:
                x_dict[ntype] = ntype_sequences[ntype][t]
            xs_encoded.append(x_dict)
    ...
    def forward(self, xs, graphs):
        ...
        # Apply GRU temporal encoding BEFORE attention
        x_win = self.encode_temporal_with_gru(x_win)

        x_res = []  # [time,ntype] -> [N,F]

        # calculation (same as original CORE)
        ATo, _, _ = self.A
        ATo = ATo.to(device)
...
class DHNet_GRU(nn.Module):
    """
    Network wrapper for DHSpace_GRU, following the same structure as DHNet.
    """
    def __init__(
        self,
        hid_dim,
    ...
    def freeze(self, module_names):
        for name in module_names:
            assert name in "feat_emb hlinear hupdate hnorm rel gru".split(), print(
        ...
        if "gru" in module_names:
            for space in self.spaces:
                if hasattr(space, 'temporal_gru') and space.use_gru:
                    for param in space.temporal_gru.parameters():
                        param.requires_grad = False
