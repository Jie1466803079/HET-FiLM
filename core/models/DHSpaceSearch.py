...
from core.models.HLinear import HLinear, HLayerNorm
...
class DHSpace(nn.Module):
    ...
    def get_modules_weights(self, target_type, source_type, ttar, tsrc, relation_type):
        ...
        # Safer guard: if AN indices exceed n_alpha rows (e.g., due to patching mismatch when loading configs),
        # clamp them into valid range to keep fix_N=False without crashes.
        if not self.fix_N:
            try:
                rows = int(self.n_alpha.size(0))
            except Exception:
                rows = 1
            try:
                FNsrc_i = int(FNsrc.item()) if torch.is_tensor(FNsrc) else int(FNsrc)
            except Exception:
                FNsrc_i = 0
            try:
                FNtar_i = int(FNtar.item()) if torch.is_tensor(FNtar) else int(FNtar)
            except Exception:
                FNtar_i = 0
            if FNsrc_i >= rows or FNsrc_i < 0:
                FNsrc_i = max(0, min(rows - 1, FNsrc_i))
            if FNtar_i >= rows or FNtar_i < 0:
                FNtar_i = max(0, min(rows - 1, FNtar_i))
            FNsrc = FNsrc_i
            FNtar = FNtar_i
...
from core.trainer import load_train_test
...
class DHSearcher:
    ...
    def _train(self, max_epochs, patience):
        ...
        is_regression = (args.dataset == "covid") or \
                        (getattr(args, 'task', '').startswith('link_weight')) or \
                        (str(getattr(args, 'predict_type', '')).lower() == 'regression')
        
        if is_regression:
            earlystop = EarlyStopping(mode="min", patience=patience)
            best_val_auc = final_test_auc = best_epoch = 1e8
            print(f"DHSearcher: Optimization mode = MINIMIZE (Regression/LinkWeight)")
        else:
            earlystop = EarlyStopping(mode="max", patience=patience)
            best_val_auc = final_test_auc = best_epoch = 0
            print(f"DHSearcher: Optimization mode = MAXIMIZE (Classification/LinkPred)")
        ...
        with tqdm(range(max_epochs), disable=self.disable_progress) as bar:
            for i in bar:
                ...
                if improved:
                    ...
                    best_epoch = i
                    print(f"best val metric : {val_auc} , saving at epoch {i}")
                    torch.save(
                        self.net.state_dict(),
